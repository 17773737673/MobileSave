...
/**
 * JazzyHelper继承抽象类OnScrollListener
 */
public class JazzyHelper implements AbsListView.OnScrollListener {

    public static final int STANDARD = 0;		//标准
    public static final int GROW = 1;			//扩展
    public static final int CARDS = 2;			//纸牌
    public static final int CURL = 3;			//卷曲
    public static final int WAVE = 4;			//波动
    public static final int FLIP = 5;			//弹跳
    public static final int FLY = 6;			//飞行
    public static final int REVERSE_FLY = 7;	//倒飞
    public static final int HELIX = 8;			//螺旋
    public static final int FAN = 9;			//扇形
    public static final int TILT = 10;			//倾斜
    public static final int ZIPPER = 11;		//拉链
    public static final int FADE = 12;			//淡入
    public static final int TWIRL = 13;			//旋转
    public static final int SLIDE_IN = 14;		//滑动

    public static final int DURATION = 300;		//持续
    public static final int OPAQUE = 255, TRANSPARENT = 0;//不透明，透明

    private JazzyEffect mTransitionEffect = null;	//接口类
    private boolean mIsScrolling = false;	//当前滚动状态
    private int mFirstVisibleItem = -1;		//第一个可见item初始值
    private int mLastVisibleItem = -1;		//最后一个可见item初始值
    private int mPreviousFirstVisibleItem = 0;//第一个之前的item是否可见
    private long mPreviousEventTime = 0;		//事件事件
    private double mSpeed = 0;				//事件速度
    private int mMaxVelocity = 0;			//最大速度
    public static final int MAX_VELOCITY_OFF = 0;//最大速度开启与关闭
    ...
    private final HashSet<Integer> mAlreadyAnimatedItems;//集合，全部准备完毕
    ...
    //2个参数构造，，可被无参构造调用 ，上下文与自定义参数
    public JazzyHelper(Context context, AttributeSet attrs) {
        mAlreadyAnimatedItems = new HashSet<Integer>();	//键值对实例
        int transitionEffect = HELIX;			//过度效果为螺旋状态
        int maxVelocity = 0;					//初始化最大速率			
        
        //避免空指针异常
        if (context != null && attrs != null) {
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.JazzyListView);//获取自定义属性
            //R.styleable.JazzyListView_effect  自定属性名称id，通过id去找到索引，STANDARD 标准模式默认值
            //反回给过度效果变量
        ...
        setTransitionEffect(transitionEffect);//设置过度效果
        setMaxAnimationVelocity(maxVelocity);	//设置最大速率
    }
    ...
    public final void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        notifyAdditionalOnScrollListener(view, firstVisibleItem, visibleItemCount, totalItemCount);//把值传递给父类
    }

    public final void onScrolled(ViewGroup viewGroup, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        boolean shouldAnimateItems = (mFirstVisibleItem != -1 && mLastVisibleItem != -1);//false

        int lastVisibleItem = firstVisibleItem + visibleItemCount - 1;	//最后项等，第一可见项+可见总项
        if (mIsScrolling && shouldAnimateItems) {				//fase&&false  应该进不去
            setVelocity(firstVisibleItem, totalItemCount);		//设置速率为，1 10
            int indexAfterFirst = 0;							//最后一个
    ...
    private void setVelocity(int firstVisibleItem, int totalItemCount) {
    	//当最大速率大于最大速率关闭		并且  之前第一条还可见不等于可见的第一条
        if (mMaxVelocity > MAX_VELOCITY_OFF && mPreviousFirstVisibleItem != firstVisibleItem) {
            long currTime = System.currentTimeMillis();//获取当前时间
            long timeToScrollOneItem = currTime - mPreviousEventTime;//当前时间-刷新时间
            //如果滚动时间小于1
            if (timeToScrollOneItem < 1) {
                double newSpeed = ((1.0d / timeToScrollOneItem) * 1000);//1/20
            ...
            mPreviousFirstVisibleItem = firstVisibleItem;//给mPreviousFirstVisibleItem赋值
            mPreviousEventTime = currTime;				//mPreviousEventTime赋值
        }
    }
